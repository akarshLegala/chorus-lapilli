...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_no_moves_after_game_end(self):
        '''Check if bottom-right button stays as blank after winning the game'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
    ...
    def test_prevent_marker_override(self):
        '''Ensure that players cannot place their marker in an already occupied tile.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
    ...
    def test_game_reset_on_refresh(self):
        '''Verify that refreshing the page resets the game board.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
    ...
    def test_middle_only_moveable_when_occupied(self):
        '''Ensure the middle tile can only be moved if occupied and both players have placed 3 markers.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[4], self.SYMBOL_BLANK)
        ...
        tiles[0].click() 
        self.assertTileIs(tiles[0], self.SYMBOL_O)
        tiles[8].click() 
